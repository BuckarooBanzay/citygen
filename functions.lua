...
function citygen.get_mapblock_from_pos(pos)
	return {
...
function citygen.get_chunkpos_from_pos(pos)
	local mapblock = citygen.get_mapblock_from_pos(pos)
...
function citygen.get_mapblocks_from_chunk(chunkpos)
	local minmapblock = {
		x = (chunkpos.x * 5) - 2,
		y = (chunkpos.y * 5) - 2,
		z = (chunkpos.z * 5) - 2,
...
function citygen.get_blocks_from_mapblock(mapblock)
	local min = {
		x = (mapblock.x) * 16,
		y = (mapblock.y) * 16,
		z = (mapblock.z) * 16
